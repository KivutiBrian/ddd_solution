...
from configs.config_sqlalchemy import SessionLocal
# schema
...
def authenticate_user(email:str, password:str, db:Session = Depends(get_db)):
    ...
    user: attendant_schema.Attendant = db.query(AttendantModel).filter(AttendantModel.EmailAddress==email).first()
    if not user:
...
def get_user(user_id:int):
    ...
    user: attendant_schema.Attendant = db.query(AttendantModel).filter(AttendantModel.Id == user_id).first()
    if not user:
        return None
    return user
...
#  get active user 
async def get_current_active_user(current_user: attendant_schema.Attendant =  Depends(get_current_user)):
    if current_user.active_status == 0:
        raise HTTPException(status_code=400, detail='Attendant account is has been retired! Contact support')
    if current_user.active_status ==  1:
        return current_user
    if current_user.active_status == 2:
        raise HTTPException(detail="Attendant account has been suspended! Contact support", status_code=400)
    if current_user.active_status == 3:
        raise HTTPException(detail="Attendant account has been blocked! This action is irreversible", status_code=400)
